...
#define NUMBER_OF_ADC_CHANNEL 10
...
uint16_t ADC_array[NUMBER_OF_ADC_CHANNEL]={0}; //Array to store the values coming from the ADC and copied by DMA
...
int main(void)
{
  ...
  GPIOB->BSRR = (1<<2); /* switch on the green led */    
//  __WFI(); /* No interrupt should occur, as only error could trigger an interrupt */ 
  GPIOB->BRR = (1<<2); /* switch off the green led */
//  DisableADC();
  SysTick_Config(48000); /* 1ms config */
  while (1) /* Infinite loop */
  {
while((ADC1->ISR & ADC_ISR_EOS) == ADC_ISR_EOS)  /* Wait end of conversion */				
{	
...
{  
  ...
  /* (2) Select output mode (01) on GPIOB pin 2 */
...
__INLINE void  ConfigureGPIOforADC(void)
{
  ...
  /* (2) Select analog mode for PA0,PA1,PA,2PA,3PA,4PA5,PA6,PA7 */
  /* (3) Select analog mode for PB0,PB1 */

  RCC->AHBENR |= RCC_AHBENR_GPIOAEN | RCC_AHBENR_GPIOBEN; /* (1) */
  GPIOA->MODER |= GPIO_MODER_MODER0 | GPIO_MODER_MODER1 | GPIO_MODER_MODER2 | GPIO_MODER_MODER3\
	| GPIO_MODER_MODER4 | GPIO_MODER_MODER5 | GPIO_MODER_MODER6 | GPIO_MODER_MODER7; /* (2) */
  GPIOB->MODER |=  GPIO_MODER_MODER0 | GPIO_MODER_MODER1; /* (3) */
...
/**
  * @brief  This function configures the ADC to convert sequentially 10 channels
            in continuous mode.
...
__INLINE void ConfigureADC(void)
{
  ...
  /* (3) Select CHSEL0, CHSEL1, CHSEL2, CHSEL3, CHSEL4, CHSEL5, CHSEL6, CHSEL7, CHSEL8 and CHSEL9 */
...
////  ADC1->CFGR1 |= ADC_CFGR1_CONT; /* (2)*/
  ADC1->CHSELR = ADC_CHSELR_CHSEL0 | ADC_CHSELR_CHSEL1  | ADC_CHSELR_CHSEL2  | ADC_CHSELR_CHSEL3  | ADC_CHSELR_CHSEL4 \
               | ADC_CHSELR_CHSEL5 | ADC_CHSELR_CHSEL6 | ADC_CHSELR_CHSEL7  | ADC_CHSELR_CHSEL8  | ADC_CHSELR_CHSEL9 ; /* (3)*/
